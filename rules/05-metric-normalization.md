...
- 输入条件：比较对象的 subject_type、层级及统计范围信息；`scope_definition` 仅为自由文本描述
- 判断逻辑：同层级且统计范围已明确一致，或经研究员确认两种范围表述等价，才继续自动同口径比较；跨集团/平台/SPV/项目须标出差异并阻断直接同口径结论。仅有相似文本、范围需语义解释或存在歧义时，不自动认定同口径
- 输出结果：`comparable_by_scope`；范围不一致或未确认时为 `comparison_blocked` / `review_required`
- 是否允许 AI 自动执行：是，仅按规则输出或执行允许的状态变化
- 是否要求人工确认：范围等价性不能直接核实时需要人工确认；确认后换算与比较可自动执行
- 示例：集团 2000 MW 与平台 1500 MW → comparison_blocked
- 例外或边界：可并列展示，但不能当同口径高低结论；AI 不得用 `scope_definition` 文本相近代替统计范围确认
